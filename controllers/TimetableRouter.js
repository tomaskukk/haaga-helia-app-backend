...
timetableRouter.get("/:tunnus", async (request, response) => {
  ...
  const firstRequest = await axios.post(
    "https://lukkarit.haaga-helia.fi/teeHaku.php",
    params
  );
  ...
  await axios.get(
    `https://lukkarit.haaga-helia.fi/paivitaKori.php?toiminto=addMulti&code=${codeParam}&viewReply=true`,
    config
  );

  const kalenteri = await axios.get(
    `https://lukkarit.haaga-helia.fi/kalenteri.php`,
    config
  );
...
timetableRouter.get("/:week/:cookie", async (request, response) => {
  try {
    ...
    const kalenteri = await axios.get(
      `https://lukkarit.haaga-helia.fi/kalenteri.php?date=${week}`,
      config
    );
