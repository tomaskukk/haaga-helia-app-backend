...
amicaRouter.get("/malmi/:lang", async (req, res) => {
  try {
    ...
    const response = await axios.get(
      `https://www.amica.fi/api/restaurant/menu/week?language=${lang}&restaurantPageId=7498&weekDate=${tomorrowAsJSON()}`
    );
...
amicaRouter.get("/haaga/:lang", async (req, res) => {
  try {
    ...
    const response = await axios.get(
      `https://www.fazerfoodco.fi/api/restaurant/menu/week?language=${lang}&restaurantPageId=244046&weekDate=${tomorrowAsJSON()}`
    );
